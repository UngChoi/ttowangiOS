...
class FullStoreTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        stores.sortInPlace({$0.heart && !$1.heart})
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let cell:StoreCell = tableView.dequeueReusableCellWithIdentifier("Cell", forIndexPath: indexPath) as! StoreCell
        ...
        if stores[indexRow].heart == true{
            if  let image = UIImage(named: "heart"){
                cell.heartButton.setImage(image, forState: .Normal)
            }
        }
        else{
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "DetailSegue"{
            ...
            if let destination = segue.destinationViewController as? DetailViewController,selectedIndex = self.tableView.indexPathForSelectedRow?.row{
